...
public class Solution {
	...
	private static int recursion(int idx, int sum, int[] num, int[][] dp) {
		if (idx == 0) {
		...
		int pick = 0;
		if (num[idx] <= sum) pick = recursion(idx - 1, sum - num[idx], num, dp);
		int skip = recursion(idx - 1, sum, num, dp);

		return dp[idx][sum] = (skip + pick) % mod;
	}
}
